...
// 채권 기간
const B_PERIODS = ['2025-12','2026-01','2026-02','2026-03','2026-04'];
const B_LABELS  = ['25-12','26-01','26-02','26-03','26-04'];
...
const TAB_META = {
  ...
  bond:    { badge:'Receivables Dashboard',  title:'채권현황 대시보드',        desc:'2026년 4월 기준 법인별 채권 구성 및 증감을 확인합니다.' },
};
...
function u_renderAll() {
  ...
  setTxt('u_selBond',  fmtEok(bEntity.periods['2026-04'].total));
  ...
  // 채권 KPI
  const bp = bEntity.periods['2026-04'];
  const bt = bp.total;
...
function u_renderBondBar(entity) {
  ...
  const data = cmp.map(e=>({ name:e.name, value:e.periods['2026-04'].total||0 }))
    .filter(d=>d.value>0).sort((a,b)=>b.value-a.value);
...
function b_select(name) {
  ...
  b_updateBtnState();
  // KPI
  const p=e.periods['2026-04'], tot=p.total;
  ...
  setTxt('b_shareTitle',`${e.name} — 2026-04 구성비`);
  ...
  const bArr=cmp.map(x=>({name:x.name,value:x.periods['2026-04'].total||0}))
    .filter(d=>d.value>0).sort((a,b)=>b.value-a.value);
  setTxt('b_barTitle',`2026-04 ${e.group==='합계'?'전체':e.group} 법인별 채권`);
  ...
  setTxt('b_momTitle',`2026-04 ${e.group==='합계'?'전체':e.group} 법인별 전월대비`);
